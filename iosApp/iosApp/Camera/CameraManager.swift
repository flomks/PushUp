...
enum DeviceLens: Sendable, Equatable {
    case front
    case back
...
final class CameraManager: NSObject, ObservableObject {
    ...
    @Published private(set) var currentPosition: DeviceLens = .back
    ...
    private var _currentPositionInternal: DeviceLens = .back
    ...
    func setupAndStart(position: DeviceLens = .back) {
    ...
    private func readPosition() -> DeviceLens {
        positionLock.lock()
        defer { positionLock.unlock() }
        return _currentPositionInternal
    }

    private func updatePosition(_ position: DeviceLens) {
        positionLock.lock()
        defer { positionLock.unlock() }
    ...
    private func configureSession(position: DeviceLens) throws {
    ...
    private func captureDevice(for position: DeviceLens) throws -> AVCaptureDevice {
