...
final class CameraManager: NSObject, ObservableObject {
    ...
    @Published private(set) var currentPosition: FacingDirection = .back
    ...
    private var _currentPositionInternal: FacingDirection = .back
    ...
    func setupAndStart(position: FacingDirection = .back) {
    ...
    private func readPosition() -> FacingDirection {
        positionLock.lock()
        defer { positionLock.unlock() }
        return _currentPositionInternal
    }

    private func updatePosition(_ position: FacingDirection) {
        positionLock.lock()
        defer { positionLock.unlock() }
    ...
    private func configureSession(position: FacingDirection) throws {
    ...
    private func captureDevice(for position: FacingDirection) throws -> AVCaptureDevice {
