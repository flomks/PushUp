...
final class WorkoutViewModel: ObservableObject {
    ...
    @Published private(set) var cameraPosition: FacingDirection = FacingDirection.front
    ...
    func resetForNewWorkout() {
        ...
        trackingManager.startCameraPreview(position: FacingDirection.front)
    }
    ...
    private func bindTrackingManager() {
        ...
        manager.cameraPositionPublisher
            .sink { [weak self] (position: FacingDirection) in self?.cameraPosition = position }
            .store(in: &cancellables)
