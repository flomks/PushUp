...
final class WorkoutViewModel: ObservableObject {
    ...
    @Published private(set) var cameraPosition: CameraPosition = CameraPosition.front
    ...
    func resetForNewWorkout() {
        ...
        trackingManager.startCameraPreview(position: CameraPosition.front)
    }
    ...
    private func bindTrackingManager() {
        ...
        manager.cameraPositionPublisher
            .sink { [weak self] (position: CameraPosition) in self?.cameraPosition = position }
            .store(in: &cancellables)
