 ...
 *
 * @property authClient     Auth API client (production: [com.sinura.data.api.SupabaseAuthClient]).
...
class AuthRepositoryImpl(
    ...
    override suspend fun loginWithApple(idToken: String): User =
        withContext(dispatcher) {
            val token = wrapAuthCall {
                authClient.signInWithIdToken(com.sinura.domain.model.SocialProvider.APPLE, idToken)
            }
            tokenStorage.save(token)
    ...
    override suspend fun loginWithGoogle(idToken: String): User =
        withContext(dispatcher) {
            val token = wrapAuthCall {
                authClient.signInWithIdToken(com.sinura.domain.model.SocialProvider.GOOGLE, idToken)
            }
            tokenStorage.save(token)
            val user = makeUserPreservingUsername(token, emailOverride = null)
            userRepository.upsertUser(user)
    ...
    private suspend fun makeUserPreservingUsername(token: AuthToken, emailOverride: String?): User {
        ...
        return makeUser(token, emailOverride).copy(
            ...
            avatarVisibility = existing?.avatarVisibility ?: com.sinura.domain.model.AvatarVisibility.EVERYONE,
        )
    }
