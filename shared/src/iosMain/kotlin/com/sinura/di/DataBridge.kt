...
import com.sinura.domain.model.RunPresenceState
...
import com.sinura.domain.usecase.SpendResult
...
object DataBridge : KoinComponent {
    ...
    private fun toRunEventResult(
        event: com.sinura.domain.model.RunEvent,
    ...
    private fun toLiveRunSessionResult(
        session: com.sinura.domain.model.LiveRunSession,
        participantCount: Int = 0,
    ...
    private fun toLiveRunParticipantResult(
        participant: com.sinura.domain.model.LiveRunParticipant,
    ...
    private fun toRunXpAwardResult(
        award: com.sinura.domain.model.RunXpAward,
                ...
                val results = events.map { event ->
                    ...
                    toRunEventResult(
                        ...
                        committedParticipantCount = participants.count {
                            it.status == com.sinura.domain.model.RunParticipantStatus.ACCEPTED ||
                                it.status == com.sinura.domain.model.RunParticipantStatus.CHECKED_IN
                        },
        ...
        scope.launch {
            val ok = runCatching {
                ...
                repository.upsertParticipant(
                    existing?.copy(
                        status = com.sinura.domain.model.RunParticipantStatus.INVITED,
                        leftAt = null,
                        finishedAt = null,
                        updatedAt = now,
                    ) ?: com.sinura.domain.model.LiveRunParticipant(
        ...
        scope.launch {
            val ok = runCatching {
                get<RunEventRepository>().updateParticipantStatus(
                    eventId = eventId,
                    userId = userId,
                    status = com.sinura.domain.model.RunParticipantStatus.CHECKED_IN,
                )
            }.isSuccess
            withContext(Dispatchers.Main) { onDone(ok) }
        }
    ...
    fun fetchRunUsers(
        ...
        scope.launch {
            try {
                val queries = get<SinuraDatabase>().databaseQueries
                val users = userIds
                    .distinct()
                    .mapNotNull { userId ->
    ...
    /**
     * Emits whenever [com.sinura.domain.model.UserSettings.dashboardWidgetOrderJson] changes locally.
     */
