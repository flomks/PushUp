...
import com.sinura.dto.DailyStatsDTO
import com.sinura.dto.MonthlyStatsDTO
import com.sinura.dto.StreakDTO
import com.sinura.dto.TotalStatsDTO
import com.sinura.dto.WeeklyStatsDTO
...
object WorkoutSessions : Table("workout_sessions") {
    val id = uuid("id")
    val userId = uuid("user_id").references(com.sinura.plugins.Users.id)
...
class StatsService {
    ...
    suspend fun getStreak(userId: UUID, today: LocalDate = LocalDate.now(ZoneOffset.UTC)): StreakDTO = newSuspendedTransaction {
        ...
        val joggingDays: List<LocalDate> = com.sinura.plugins.JoggingSessions
            .select(com.sinura.plugins.JoggingSessions.startedAtDay)
            .where {
                (com.sinura.plugins.JoggingSessions.userId eq userId) and
                    com.sinura.plugins.JoggingSessions.endedAt.isNotNull()
            }
            .withDistinct()
            .orderBy(com.sinura.plugins.JoggingSessions.startedAtDay to SortOrder.ASC)
            .map { row: ResultRow ->
                row[com.sinura.plugins.JoggingSessions.startedAtDay].atZone(ZoneOffset.UTC).toLocalDate()
            }
